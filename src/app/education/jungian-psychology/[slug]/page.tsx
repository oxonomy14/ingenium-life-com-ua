...
export async function generateMetadata({
  ...
  const course = getCourse('jungian-psychology', slug);

  if (!course) {
    return {};
  }

  const canonical = `/education/jungian-psychology/${course.slug}`;
...
export default async function CoursePage({ params }: CoursePageProps) {
  ...
  return (
    <>
      {' '}
      <LayoutShell>
        <CourseSchema course={course} categorySlug="jungian-psychology" />

        <CourseBreadcrumbSchema
          course={course}
          categorySlug="jungian-psychology"
          categoryName="Юнгианская психология"
        />
